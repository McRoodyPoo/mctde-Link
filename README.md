...
> **Enable PhantomUnleashed?** This raises the co-op / invasion phantom cap above the stock 4.
> While enabled you will **only** be able to connect with other players who also have
> PhantomUnleashed enabled. Choose **No** to play with everyone on the normal phantom limit.

- **No** — nothing is patched; you stay in the normal pool and can connect with everyone.
- **Yes** — the phantom-cap patches are applied and you join the **segregated PhantomUnleashed
  pool**, so a vanilla player is never pulled into a larger-than-four session.

### Configuration

The `[PhantomUnleashed]` section of `mctde-link.ini` controls it:
...
allocation failures. PhantomUnleashed raises this pool (default 192 MB, capped at a safe 255 MB)
...
must also change this version. PhantomUnleashed writes `NetworkVersion` (default `0x4D`) at the
game's three version-check sites, so a PhantomUnleashed client only ever connects to other
PhantomUnleashed clients sharing the same value. (Reverse engineering of the version sites:
...
written to `PhantomUnleashed.log` next to `d3d9.dll` regardless of the `[Settings] EnableLogging`
switch.

> **Status / safety:** this feature ships with `VerifyOnly=1`. In that mode it only writes a
> verification report to `PhantomUnleashed.log` and never modifies the running game. Set
...
> running them together double-patches the game and crashes. PhantomUnleashed detects a loaded or
