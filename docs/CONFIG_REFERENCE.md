...
Total players in your world (stock 4). Keep at `18` for a stable large session; the offset-shift trampolines are calibrated for 18.

`NetworkVersion=0x4D`

Matchmaking pool key (vanilla retail is `0x2E`). Everyone in a session must share this and `MaxPhantoms`. Set `0x2E` to rejoin the vanilla pool.

`MemoryPoolMB=192`

Raises the game's internal memory pool (stock ~10 MB is too small for many phantoms). Range `0`–`255`; `0` leaves it stock.
...
Legacy layout toggle. It is still read by the config loader, but the current compact overlay does not draw a header, so this has no visible effect in v0.1.1.
...
Windows virtual-key codes (decimal or hex) for the panel's hide bind. Defaults are Shift (`0x10`) + F2 (`0x71`). Set the modifier to `0` for a no-modifier bind. The on-screen text always reads "Press SHIFT + F2", so keep these in sync if you rebind.
...
These keys may exist in older test configs but are not read by the v0.1.1 source:
