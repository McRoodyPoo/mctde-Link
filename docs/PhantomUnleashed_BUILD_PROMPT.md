...
- **Before the game opens**, prompt the player: *"Enable PhantomUnleashed? (You will only be able to connect
  with other players who also have PhantomUnleashed enabled.)"* with two buttons: Yes / No.
...
- **No unbounded scans.** The reference uses `while(scan==NULL){Sleep(100);}`. Replace it with a bounded
...
2. **2 code-cave detours:**
   ...
   - plus static `patch35`: `mov ecx,[edi+0xE4]` at `ds1_base + 0xAA277A` (fixed bytes
     `8B 8F E4 00 00 00`). This is part of the pca offset-reference fixes.
...
- **Author your own trampolines** (your own register/scratch choices, your own cave allocation that
  ...
  the exact asm text is the gray area, so write it yourself. Make the trampolines reversible (track caves so
...
1. **Fold in (Req A)**, build mctde-Link Release|Win32. Confirm it still loads and the overlay works
   (no behavior change yet, with `Mode=Off` or `VerifyOnly=1`).
